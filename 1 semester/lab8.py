#Цель: табулирование данной функции y=f(x), где х = a*b^2 - sin(b^2) +m*b с помощью оператора цикла с предусловием
#Переменные:
...
#Метод: ввод функции пользователя для вычисления значения f(x), использование кодировки Unicode ля вызова символов псевдографики
...
if ((an>a0 and ax>0) or (an<a0 and ax<0)):
    ...
    while ((a<=an) and ax>0) or ((a>=an) and ax<0):
        x=a*b*b-sin(b*b)+m*b
        print(chr(9500) + chr(9472) * 20 + chr(9532) + chr(9472) * 20 + chr(9532) + chr(9472) * 20 + chr(9508))
        print(chr(9474) + str(f(x)) + ' ' * (20 - len(str(f(x)))) + chr(9474)
              +str(round(x, 5)) + ' ' * (20 - len(str(round(x, 5))))+chr(9474) + str(round(a, 5)) + ' ' * (20 - len(str(round(a, 5)))) + chr(9474))
        a=round(a+ax, 5)
